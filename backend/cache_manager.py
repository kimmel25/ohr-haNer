...
This saves both Claude API calls and Sefaria API calls to disk, so you don't
pay for the same request twice.
"""
    ...
    def get(self, key: str) -> Optional[dict]:
        ...
        try:
            ...
            if datetime.now() - cached_time > self.ttl:
                logger.info(f"Cache EXPIRED: {key} (cached {cached_time})")
                os.remove(cache_path)
                return None

            logger.info(f"Cache HIT: {key} (cached {cached_time})")

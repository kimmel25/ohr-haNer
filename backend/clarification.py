...
async def generate_clarification_options(
    ...
    # Otherwise, ask Claude to generate options
    try:
        ...
        max_tokens = min(
            getattr(settings, "claude_clarification_max_tokens", 1500),
            1500,
        )
        response = client.messages.create(
            ...
            system="You are a Torah learning assistant. Help clarify ambiguous queries by generating distinct interpretation options.",
        ...
        logger.info(f"[CLARIFICATION] Generated {len(options)} options from Claude")

        return ClarificationResult(
            needs_clarification=True,
            question=data.get("question", "Which interpretation did you mean?"),
            options=options,
            reason="Generated by Claude",
        )
...
async def check_and_generate_clarification(
    ...
    logger.info("[CLARIFICATION] No pre-computed options, falling back to Claude API call")
